...
echo "------------------------------------------------------------------------------"
echo "Our GLOBAL Templates are getting bigger than 100,000 bytes, so we need to minify them"
echo "------------------------------------------------------------------------------"
...
# Create the output file and add the header
{
    ...
    echo "# Generated date and time: $current_datetime    $NAMESPACE"
} > $output_file

# Append the AWS CLI command to the output file
./format-aws-cli.sh << EOF >> $output_file
...
--parameters \\
  ...
  ParameterKey=SecHubAdminAccount,ParameterValue=$SECHUB_ACCOUNT \\
...
# Define output file
output_file='awscli-testcreatemember-stack.sh'
echo "Building $output_file"
current_datetime=$(date) 
...
# Define output file
output_file='awscli-testcreatemember-roles.sh'
echo "Building $output_file"
current_datetime=$(date) 
...
# Define output file
output_file='awscli-update-mpa-member-roles.sh'
echo "Building $output_file"
current_datetime=$(date) 
