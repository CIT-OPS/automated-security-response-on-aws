...
def runbook_handler(event,context):
    ...
    responses["EnableSNSDeliveryLoggingResponse"].append('SNSSuccessFeedback is '+SNSSuccessFeedback)
    responses["EnableSNSDeliveryLoggingResponse"].append('SNSFailureFeedback is '+SNSFailureFeedback)
    ...
    responses["EnableSNSDeliveryLoggingResponse"].append('Setting ApplicationSuccessFeedbackRoleArn '+json.dumps(response['ResponseMetadata']))
    response = client.set_topic_attributes(
        TopicArn=ResourceId,
        AttributeName='ApplicationSuccessFeedbackSampleRate',
        AttributeValue="100"
    )
    responses["EnableSNSDeliveryLoggingResponse"].append('Setting ApplicationSuccess Rate to 100 '+json.dumps(response['ResponseMetadata']))
    response = client.set_topic_attributes(
        TopicArn=ResourceId,
        AttributeName='ApplicationFailureFeedbackRoleArn',
        AttributeValue=SNSFailureFeedback
    )
    responses["EnableSNSDeliveryLoggingResponse"].append('Setting ApplicationFailureFeedbackRole Arn '+json.dumps(response['ResponseMetadata']))
    print(responses)
    return responses
